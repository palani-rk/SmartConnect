...
export class ScreenshotProcessor {
  ...
  async prepareForAnalysis(screenshotPath: string): Promise<ScreenshotAnalysisData> {
    ...
    const screenshots = await Promise.all(
      screenshotFiles.map(file => this.processScreenshot(file))
    );
    ...
    return {
      screenshots: screenshots.filter(s => s !== null) as any[],
      summary
    };
  }

  private async findScreenshots(dirPath: string): Promise<string[]> {
    try {
      const patterns = [
        '**/*.png',
        '**/*.jpg',
  ...
  private async processScreenshot(filePath: string): Promise<{
    path: string;
    scenario: string;
    browser: string;
    viewport: string;
  ...
  private formatScenarioName(pageName: string, scenarioId: string): string {
  ...
  private formatViewportName(viewport: string): string {
  ...
  private generateSummary(screenshots: any[]): {
    totalTests: number;
    passed: number;
    failed: number;
    skipped: number;
    ...
    const failed = screenshots.filter(s => 
      s.path.includes('_error') || s.path.includes('_failed')
    ).length;
  ...
  generateAnalysisPrompts(screenshots: ScreenshotAnalysisData): string[] {
    ...
    prompts.push(
      `I've captured ${screenshots.summary.totalTests} UX test screenshots. ` +
  ...
  async compareWithBaseline(currentPath: string, baselinePath: string): Promise<{
    comparisons: Array<{
      scenario: string;
      current: string;
      baseline: string;
    ...
    for (const currentFile of currentScreenshots) {
      ...
      const baselineFile = baselineScreenshots.find(baseline => 
        path.basename(baseline) === currentName
      );
  ...
  async cleanupOldScreenshots(dirPath: string, maxAge: number = 7): Promise<void> {
    try {
  ...
  async createThumbnails(screenshotPath: string, thumbnailPath: string): Promise<void> {
